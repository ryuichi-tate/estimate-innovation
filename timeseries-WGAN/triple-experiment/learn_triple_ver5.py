...
tiple1と比べて、predictorの学習をもっと少なくした。
'''
# %matplotlib inline
import argparse
import os
...
for epoch in range(1, opt.n_epochs+1):# epochごとの処理(discriminatorのepoch)
    ...
    for i, batch in enumerate(batch_sample):
        ...
        X = trainMatrix[batch : batch+opt.batch_size]# torch.Size([64, 1, 8]) (batch, dim, p+1)
        # X = Variable(X)# 自動微分可能に。(もういらないらしい？orz)
    ...
    if saveModel and epoch % opt.sample_interval == 0:
        torch.save(generator.state_dict(), 'parameters/p'+str(p)+'/No{5}_generator_epoch{0}_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(epoch, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No))
        torch.save(discriminator.state_dict(), 'parameters/p'+str(p)+'/No{5}_discriminator_epoch{0}_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(epoch, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No ))
        torch.save(predictor.state_dict(), 'parameters/p'+str(p)+'/No{5}_predictor_epoch{0}_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(epoch, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No ))
    ...
    if min_floss > val_loss_F_curve[-1]:
        ...
        torch.save(generator.state_dict(), 'parameters/p'+str(p)+'/No{5}_generator_minLoss_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(epoch, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No))
        torch.save(discriminator.state_dict(), 'parameters/p'+str(p)+'/No{5}_discriminator_minLoss_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(epoch, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No ))
        torch.save(predictor.state_dict(), 'parameters/p'+str(p)+'/No{5}_predictor_minLoss_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(epoch, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No ))
