...
for epoch in range(1, pretrain_epoch+1):# epochごとの処理
    ...
    if epoch % 100==0:
        ...
        plt.close()
torch.save(predictor.state_dict(), 'parameters/p'+str(p)+'/No{5}_predictor_epoch{0}_batchSize{1}_GP{2}_Corr{3}_DataSeed{4}.pth'.format(0, opt.batch_size, int(gp_weight), int(corr_weight), dataSeed, No ))
print("pre-training終了")

# hat_sigmaに相当する部分が0になってるので1にする
