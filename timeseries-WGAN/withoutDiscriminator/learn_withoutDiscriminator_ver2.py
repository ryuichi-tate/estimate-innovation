...
min_gloss=np.inf# epochのflossのの最小値を保管
start=time.time()
...
#     val_loss_D_curve.append(float(val_loss_D))
    ...
    if min_floss > val_loss_F_curve[-1]:
        ...
        torch.save(generator.state_dict(), 'parameters/p'+str(p)+'/No{0}_generator_minfLoss_batchSize{2}_DataSeed{3}.pth'.format(No, epoch, opt.batch_size, dataSeed))
        torch.save(predictor.state_dict(), 'parameters/p'+str(p)+'/No{0}_predictor_minfLoss_batchSize{2}_DataSeed{3}.pth'.format(No, epoch, opt.batch_size, dataSeed))
        print("validationのflossの最小値を更新しました　　Loss:", min_floss)
    
    # val_loss_Gの最小値を保管
    if min_gloss > val_loss_G_curve[-1]:
        min_gloss=val_loss_G_curve[-1]
        torch.save(generator.state_dict(), 'parameters/p'+str(p)+'/No{0}_generator_mingLoss_batchSize{2}_DataSeed{3}.pth'.format(No, epoch, opt.batch_size, dataSeed))
        torch.save(predictor.state_dict(), 'parameters/p'+str(p)+'/No{0}_predictor_mingLoss_batchSize{2}_DataSeed{3}.pth'.format(No, epoch, opt.batch_size, dataSeed))
        print("validationのglossの最小値を更新しました　　Loss:", min_gloss)
